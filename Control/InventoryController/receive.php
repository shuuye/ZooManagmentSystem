...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if (!empty($POid) && is_numeric($POid)) {
        ...
        if ($result) {
            header('Location: index.php?controller=inventory&action=showPO&status=updateSuccess');
        } else {
            header('Location: index.php?controller=inventory&action&status=updateError');
        }
