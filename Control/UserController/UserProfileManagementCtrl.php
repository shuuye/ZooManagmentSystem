<?php
    ...
    class UserProfileManagementCtrl extends UserController{
        ...
        private function validateCurrentPassword(){
            if (InputValidationCtrl::inputIsEmptyValidation($this->currentPassword)) { //check empty
                return 'Current Password cannot be empty!';
            }
            
            if(!$this->authUserInDB($this->username, $this->currentPassword)){ //the curent password should able to auth with username of logged in user
        ...
        private function validateNewPassword() {
            ...
            if (!InputValidationCtrl::inputMinLengthValidation($this->newPassword, 6)) { //the new password required to be 6 character or more
        ...
        private function validateConfirmPassword() {
            ...
            if (!InputValidationCtrl::inputMatchValidation($this->confirmPassword, $this->newPassword)) { //the confirm password should matched with the new password
        ...
        private function validateNewFullName() {
            if (InputValidationCtrl::inputIsEmptyValidation($this->newFullName)) {//check empty
                return 'New Name cannot be empty!';
            }

            if (!InputValidationCtrl::inputFormatValidation($this->newFullName, "/^[a-zA-Z ]*$/")) { //should only allowed letter and space
        ...
        private function validateNewPhoneNumber() {
            if (InputValidationCtrl::inputIsEmptyValidation($this->newPhoneNumber)) {//check empty
                return 'New Phone Number cannot be empty!';
            }

            if (!InputValidationCtrl::inputFormatValidation($this->newPhoneNumber, "/^\d{10,11}$/")) { //only allow 10 to 11 digit without dash
        ...
        private function resetCurrentUserModel() {
            ...
            //update the current user session by role id
        ...
        private function afterResetPasswordInput($validInput) {
            ...
            //hash the password b4 pass in to database
