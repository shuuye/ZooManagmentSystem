...
<?php
    ...
    class StaffCtrl extends UserController{
        ...
        private function validateAttendanceDateTime() {
            ...
            $workingStartingTime = $this->workingStartingTime;
            $workingOffTime = $this->workingOffTime;
            $attendanceDateTime = $this->attendanceDateTime;

            if (empty($workingDate) || empty($workingStartingTime) || empty($workingOffTime) || empty($attendanceDateTime)) {
                return 'Required fields are missing.';
            }

            // Combine date and time into full datetime strings
            $startDateTime = $workingDate . ' ' . $workingStartingTime;
            $endDateTime = $workingDate . ' ' . $workingOffTime;

            // Convert to timestamps
            $startTimestamp = strtotime($startDateTime);
            $endTimestamp = strtotime($endDateTime);
            $attendanceTimestamp = strtotime($attendanceDateTime);
        ...
        private function setAttendanceTemp() {
            ...
            $this->attendanceDateTime = date('Y-m-d H:i:s');
        ...
        private function checkAttendanceInput(){
            $data = [
                ...
                'attendanceDateTimeErr' => $this->validateAttendanceDateTime(),
        ...
        private function afterAttendanceInput($validInput){
            ...
            if ($validInput) {
                ...
                if (is_string($savedPhotoPath) && strpos($savedPhotoPath, 'Failed') === false) {
                    ...
                    $attendanceModel = new AttendanceModel();
                    $updatedStatus = $attendanceModel->updateStatusIDFromAttendanceByPrimaryKey($this->id, $this->workingDate, $this->workingStartingTime, $this->workingOffTime, 2);
                    $updatePhoto = $attendanceModel->updateColumnFromAttendanceByPrimaryKey($this->id, $this->workingDate, $this->workingStartingTime, $this->workingOffTime, 'photo', $this->photo);
                    $updateLocation = $attendanceModel->updateColumnFromAttendanceByPrimaryKey($this->id, $this->workingDate, $this->workingStartingTime, $this->workingOffTime, 'location', $this->location);
                    $updateAttendanceDateTime = $attendanceModel->updateColumnFromAttendanceByPrimaryKey($this->id, $this->workingDate, $this->workingStartingTime, $this->workingOffTime, 'attendanceDateTime', $this->attendanceDateTime);
...
>>>>>>> d2735169db0cad24f03b69a481318eac092e633a
