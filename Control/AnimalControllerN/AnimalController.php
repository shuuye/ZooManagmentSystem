...
//        $this->handleRequest();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($inventoryId) {
            ...
            $animalId = $this->animalModel->addAnimal($inventoryId, $animalDetails);

            if ($animalId) {
                $message = "Animal added successfully.";
            } else {
                $message = "Failed to add animal.";
                $animalId = null; // Ensure $animalId is set to null if animal addition fails
            }
                    ...
                    if (!in_array($fileExtension, ['jpg', 'jpeg', 'png'])) {
                        ...
                        if (!is_dir($uploadDir) || !is_writable($uploadDir)) {
                            ...
                            if (move_uploaded_file($file['tmp_name'], $uploadFilePath)) {
                                // Perform virus scan (placeholder for ClamAV)
                                // Implement actual virus scanning here

                                // Save file information in the database
                                $imagePath = '/assests/AnimalImages/' . $uniqueFileName;
                                if ($animalId) {
